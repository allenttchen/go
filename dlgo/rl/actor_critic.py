...
class ACAgent(Agent):
    """
    Actor-Critic Agent
    The actor-critic model has one input and two outputs. The input is a representation of the board state.
    The outputs are a probability distribution over moves and expected return from the current position (Value output).
    Training Data:
    1. Input: board encoding
    2. Output 1 (dist): vector of chosen move with advantage (instead of 1 in one-hot or 1 or -1 in policy gradient)
    3. Output 2 (v): exactly like the training target for Q-learining (1 for win, -1 for loss)
    """
